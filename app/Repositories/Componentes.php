...
class Componentes{
	...
	static function geraCarteiraPadrao($empresaID,$usersId,$nomeCarteira='carteira_padrao',$moeda_id='USD',$pode_alterar=1){
		$dados = [
					...
					'hash' => 'A'.$moeda_id.Componentes::geraNumeroCarteira(),
				];
	...
	static function MontaBotao($data){
		...
		switch ($tipo) {
			...
			case 'modalGeral':
					...
					$botao .= '<li class="btn btn-xs '.$cor.'" data- data-toggle="tooltip" title="'.$titulo.'" style="'.$style.'">';
					$botao .= $icone;
					$botao .= $label;
					$botao .= '</li>';
					$botao .= '</a>';
			...
			case 'linkDireto':
				...
				<li data- data-toggle="tooltip" title="'.$titulo.'" style="list-style: none; '.$style.'">
				'.$icone.' &nbsp; 
				'.$label.'
				</li>
				</a>';
			...
			case 'LinkGeral':
				if( $target ){
					...
					<li class="btn btn-sm btn-block btn-'.$cor.'" data- data-toggle="tooltip" title="'.$titulo.'" style="'.$style.'">
					'.$icone.' &nbsp; 
					'.$label.'
					</li>
					</a>
					...
					$botao = '
					<a onclick="'.trataUrlparaFuncao($url).';" style="cursor: pointer">
					<li class="btn btn-sm btn-block btn-'.$cor.'" data- data-toggle="tooltip" title="'.$titulo.'" style="'.$style.'">
					'.$icone.'
					'.$label.'
					</li>
			...
			case 'BotaoModalSalvar':
				$botao = '';
				if( $tamanhoCheio === 1 ){
				$botao .= '<div class="form-group row" style="'.$styleDiv.'">';
			...
			case 'BotaoRemover':
				...
				switch ($alertBotaoRemover) {
					case 'desativar':
/* 0 */					$retornoBotao .= 'title:' . trataTraducoes('Atenção') . '|';
/* 1 */					$retornoBotao .= 'text:' . trataTraducoes('Deseja desativar esse registro') . '|';
/* 2 */					$retornoBotao .= 'type:' . 'warning' . '|';
/* 3 */					$retornoBotao .= 'confirmButtonColor:' . '#DD6B55' . '|';
/* 4 */					$retornoBotao .= 'confirmButtonText:' . trataTraducoes('Sim') . '|';
/* 5 */					$retornoBotao .= 'cancelButtonText:' . trataTraducoes('Não') . '|';
					break;
					
					case 'reativar':
/* 0 */					$retornoBotao .= 'title:' . trataTraducoes('Atenção') . '|';
/* 1 */					$retornoBotao .= 'text:' . trataTraducoes('Deseja reativar esse registro') . '|';
/* 2 */					$retornoBotao .= 'type:' . 'warning' . '|';
/* 3 */					$retornoBotao .= 'confirmButtonColor:' . '#DD6B55' . '|';
/* 4 */					$retornoBotao .= 'confirmButtonText:' . trataTraducoes('Sim') . '|';
/* 5 */					$retornoBotao .= 'cancelButtonText:' . trataTraducoes('Não') . '|';
					...
					default:
/* 0 */					$retornoBotao .= 'title:' . trataTraducoes('Atenção') . '|';
/* 1 */					$retornoBotao .= 'text:' . trataTraducoes('Confirma remoção do registro') . '|';
/* 2 */					$retornoBotao .= 'type:' . 'warning' . '|';
/* 3 */					$retornoBotao .= 'confirmButtonColor:' . '#DD6B55' . '|';
/* 4 */					$retornoBotao .= 'confirmButtonText:' . trataTraducoes('Sim') . '|';
/* 5 */					$retornoBotao .= 'cancelButtonText:' . trataTraducoes('Não') . '|';
				...
				$botao = '<a '.mostraToolTip($titulo).' data-url="'.$url.'" data-alert="'.$retornoBotao.'" data-token='.csrf_token().' title="'.$label.'" class="btn-apagar btn '.$cor.' '.( $cor != 'btn-white' ? 'text-white' : Null ).' btn-xs float-right" data-toggle="tooltip" data-placement="top" style="margin: 0px 2px 0px 2px !important;">'.$icone.' '.$label.'</a>';
				break;

			case 'LinkGeralIcone':
			if( empty($data['filhos']) ){
				if( $target ){
					$botao = '<a '.mostraToolTip($titulo).' href="'.$url.'" target="'.$target.'" title="'.trataTraducoes($label).'" class="btn-apagar btn '.$cor.' '.( $cor != 'btn-white' ? 'text-white' : Null ).'  btn-xs float-right '.$classHref.'"  data-toggle="tooltip" data-placement="top" style="margin: 0px 2px 0px 2px !important; '.$styleHref.'">'.$icone.' '.trataTraducoes($label).'</a>';
				} else {
					$botao = '<a '.mostraToolTip($titulo).' onclick="'.trataUrlparaFuncao($url).';" title="'.trataTraducoes($label).'" style="float: '.( !empty($data['float']) ? $data['float'] : 'right' ).' !important; margin: 0px 2px 0px 2px !important; '.$styleHref.'" class="btn-apagar btn '.$cor.' '.( $cor != 'btn-white' ? 'text-white' : Null ).' btn-xs float-right '.$classHref.'" data-toggle="tooltip" data-placement="top"> '.$icone.' <span style="'.$styleSpan.'">'.trataTraducoes($label).'</span></a>';
				}
			} else {
				$botao = '<a '.mostraToolTip($titulo).' class="dropdown-toggle btn '.$cor.' '.( $cor != 'btn-white' ? 'text-white' : Null ).' btn-xs float-right '.$classHref.'" data-toggle="dropdown" href="#" style="margin: 0px 2px 0px 2px !important; '.$styleHref.'">'.$icone.' '.trataTraducoes($label).'</a><ul class="dropdown-menu dropdown-user">';

				foreach( $data['filhos'] as $key => $filhos ){
					$botao .= '<li style="margin: 0px 2px 0px 2px !important; '.$styleLi.'"><a '.mostraToolTip($titulo).' onclick="'.trataUrlparaFuncao($filhos['url']).'" class="dropdown-item"> <i class="'.$filhos['icone'].'"> </i> '.trataTraducoes($filhos['label']).'</a></li>';
				}
				$botao .= '</ul>';
			}
				break;
	...
	static function botaoModalPerson($id, $texto = ''){
		if( !empty($id) ){
			...
			$texto = ( !empty($texto) ? $texto : trataTraducoes('Confirma ação') );
			...
			function reativaVeiculo(".$id.") {
				swal({
					...
					cancelButtonText: '".trataTraducoes('Não')."',
	...
	static function geraImagem($formato='',$largura='',$altura='',$cor='',$conteudo=''){
		...
		// The text to draw
		$text = trataTraducoes('Comprovante de transferência');
	...
	static function botaoHabilitaDesabilita($data){
		...
		$botao_cancelamento = ( !empty($data['botao_cancelamento']) ? $data['botao_cancelamento'] : trataTraducoes('Não') );
	...
	static function atualizaCotacaoMoedas(){
		...
		foreach( $retorno['quotes'] as $key => $data ){
		    MoedasConversoes::create([
		        'moeda_origem' => $retorno['source'],
		        'moeda_destino' => $key,
		        'valor' => $data,
	...
	static function montaQuadrosSenhaFinanceira($data=''){
		...
		$inicio = 1;
		while ($inicio <= $qdade) {
		$campos .= '<input '. ($inicio===1 ? ' autofocus="autofocus" ' : '') .' pattern="([0-9]{1})" autocomplete="off" required="required" value="" maxlength="'.$maxlength.'" name="'.$campoName.'[]" id="'.$campoName.'[]" type="'.$tipo.'" class="form-control '.$campoName.'_salta" placeholder="" style="float: left; width: '.$larguraCampos.'% !important; text-align: center !important;">
					';
			$inicio++;
		}
