...
class ContasReceber_Repositorie_Campos{

    const url = '/veiculos/painel/financeiro/contas_receber';

    static function index($id = ''){

        $dados = [
            'rota_geral'=>ContasReceber_Repositorie_Campos::url,
            'rota_geral_voltar'=>ContasReceber_Repositorie_Campos::url,
            'titulo_pagina'=>'Início|Veículos|Financeiro|Contas a receber',
            'botoes_da_datatable'=>Componentes::MontaBotao([
                'cor'=>'primary',
                'url'=>ContasReceber_Repositorie_Campos::url.'/create',
                'tipo'=>'LinkGeralIcone',
        ...
        $data = Model('Veiculos','Financeiro')::with('credor')->where('tipo','contas_receber')->orderby('vencimento')->get();
        ...
        foreach( $data as $key=>$d ){
            ...
            if( is_null($d['data_pagamento']) ){
                $d['acoes'] .= Componentes::MontaBotao(['cor'=>'danger','url'=>ContasReceber_Repositorie_Campos::url.'/'.$d['hash_id'].'','tipo'=>'BotaoRemover','icone'=>'fa fa-trash','titulo'=>'Remover','classHref'=>'botaoRemover']);

                $d['acoes'] .= Componentes::MontaBotao(['cor'=>'warning','url'=>ContasReceber_Repositorie_Campos::url.'/'.$d['hash_id'].'/edit','tipo'=>'LinkGeralIcone','icone'=>'fa fa-pencil','titulo'=>'Editar']);

                $d['acoes'] .= Componentes::MontaBotao(['cor'=>'info','url'=>ContasReceber_Repositorie_Campos::url.'/'.$d['hash_id'].'/pagar','tipo'=>'LinkGeralIcone','icone'=>'fa fa-download','titulo'=>'Baixar']);
            } else {
                $d['acoes'] .= Componentes::MontaBotao(['cor'=>'info','url'=>ContasReceber_Repositorie_Campos::url.'/'.$d['hash_pagamento'].'/recibo','tipo'=>'LinkGeralIcone','icone'=>'fa fa-file-pdf-o','titulo'=>'Recibo','target' => '_blank']);
            }
        } 
    ...
    static function createorEdit($id = ''){
        ...
        $formulario[] = FormularioRepositorie::formulario([
            ...
            'iconeD' => ['tipo'=>'icone','arquivo'=>'fa fa-plus','url'=>'/veiculos/painel/cadastros/clientes/create'],
...
*/
        ...
        $dados = ContasReceber_Repositorie_Campos::index()['dados'];
    ...
    static function pagar($id){
        ...
        $formulario[] = FormularioRepositorie::formulario([
            ...
            'mascara'=>'mascaraMoeda',
            'icone' => ['tipo'=>'icone','tipo'=>'letra','arquivo'=>$moedaPadrao],
            'valor_inicial' => ( !empty($data['valor_pago']) ? currencyToSystemDefaultBD($data['valor_pago'],( strtolower($data['moeda']) === 'pyg' ? 3 : 2 )) : 0 )
        ]);
