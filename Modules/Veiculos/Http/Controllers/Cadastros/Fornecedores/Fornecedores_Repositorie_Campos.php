...
class Fornecedores_Repositorie_Campos{

    const url = '/veiculos/painel/cadastros/fornecedores';

    static function index($id = ''){
        $dados = [
            'rota_geral'=>Fornecedores_Repositorie_Campos::url,
            'titulo_pagina'=>'Início|Cadastros|Fornecedores',
            'botoes_da_datatable'=>Componentes::MontaBotao([
                'cor'=>'primary',
                'url'=>Fornecedores_Repositorie_Campos::url.'/create',
                'tipo'=>'LinkGeralIcone',
        ...
        $data = Model('Veiculos','Cadastros')::where('tipo','for')->orderby('nome')->get();
        ...
        foreach( $data as $key => $d ){
            ...
            $d['acoes'] .= Componentes::MontaBotao(['cor'=>'danger','url'=>Fornecedores_Repositorie_Campos::url.'/'.$d['hash_id'].'','tipo'=>'BotaoRemover','icone'=>'fa fa-trash','titulo'=>'Remover','classHref'=>'botaoRemover']);
            $d['acoes'] .= Componentes::MontaBotao(['cor'=>'warning','url'=>Fornecedores_Repositorie_Campos::url.'/'.$d['hash_id'].'/edit','tipo'=>'LinkGeralIcone','icone'=>'fa fa-pencil','titulo'=>'Editar']);
        } 
            ...
            [
                ...
                'formulario' => [
                    ...
                    FormularioRepositorie::formulario([
                        'formulario' => 9,
                        'label' => 'Nome fantasia',
                        'nome_no_banco_de_dados' => 'fantasia',
                        'icone' => ['tipo'=>'icone','arquivo'=>'fa fa-user'],
                        'valor_inicial' => ( !empty($data['fantasia']) ? $data['fantasia'] : '')
                    ]),
                    FormularioRepositorie::formulario([
                        'formulario' => 5,
                        'label' => 'Email',
...
$formRequest = [
    'nome|Nome'=>['required'=>'Campo obrigatório','min:1'=>'Campo de deve ter no mínimo |min| caracteres',],
    'email|Email'=>['required'=>'Campo obrigatório','min:1'=>'Campo de deve ter no mínimo |min| caracteres',],
];

$dados = Fornecedores_Repositorie_Campos::index()['dados'];
