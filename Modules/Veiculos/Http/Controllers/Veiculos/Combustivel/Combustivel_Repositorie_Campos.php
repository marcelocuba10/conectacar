...
class Combustivel_Repositorie_Campos{

    const url = '/veiculos/painel/veiculos/combustivel';

    static function index($id = ''){

        $dados = [
            'rota_geral'=>Combustivel_Repositorie_Campos::url,
            'titulo_pagina'=>'Início|Veículos|Combustivel',
            'botoes_da_datatable'=>Componentes::MontaBotao([
                'cor'=>'primary',
                'url'=>Combustivel_Repositorie_Campos::url.'/create',
                'tipo'=>'LinkGeralIcone',
                'titulo'=>'Cadastrar novo combustível',
            ]),
            'botaoPDF'=>0,
            'botaoExcel'=>0,
            'botaoImprimir'=>0,
        ...
        $datatable = [
            ...
            ['tabela'=>85,'label'=>'Combustível','nome_no_banco_de_dados'=>'nome',],
            ['tabela'=>10,'label'=>'Ações','nome_no_banco_de_dados'=>'acoes',]
        ];

        $data = Model('Veiculos','VeiculosVariacoes')::where('tipo','combustivel')->orderby('nome')->get();

        foreach( $data as $key=>$d ){
            $d['contador'] = ($key+1);
            $d['acoes'] = '';
            if( $d['emp_id'] === Auth()->user()->emp_id ){
                $d['acoes'] .= Componentes::MontaBotao(['cor'=>'danger','url'=>Combustivel_Repositorie_Campos::url.'/'.$d['id'].'','tipo'=>'BotaoRemover','icone'=>'fa fa-trash','titulo'=>'Remover','classHref'=>'botaoRemover']);
                $d['acoes'] .= Componentes::MontaBotao(['cor'=>'warning','url'=>Combustivel_Repositorie_Campos::url.'/'.$d['id'].'/edit','tipo'=>'LinkGeralIcone','icone'=>'fa fa-pencil','titulo'=>'Editar']);
            }
        } 
...
*/
        ...
        $dados = Combustivel_Repositorie_Campos::index()['dados'];
